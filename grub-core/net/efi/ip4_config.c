...
grub_efi_string_to_ip4_address (const char *val, grub_efi_ipv4_address_t *address, const char **rest)
{
  ...
  for (i = 0; i < 4; i++)
    {
      unsigned long t;
      t = grub_strtoul (ptr, (const char **) &ptr, 0);
      if (grub_errno)
	{
...
grub_efi_ip4_interface_route_table (struct grub_efi_net_device *dev)
{
  ...
  interface_info = efi_ip4_config_interface_info (dev->ip4_config);
  if (interface_info == NULL || interface_info->route_table == NULL)
    return NULL;

  if (grub_add (interface_info->route_table_size / sizeof (grub_efi_ip4_route_table_t), 1, &nmemb))
    {
      grub_errno = GRUB_ERR_OUT_OF_RANGE;
      return NULL;
    }
  ...
  for (i = 0; i < interface_info->route_table_size / sizeof (grub_efi_ip4_route_table_t); i++)
    {
	...
	ret[i] = grub_xasprintf ("%s:local %s/%d %s", dev->card_name, subnet, mask_size, interface_name);
      else if (u32_subnet && u32_gateway)
	ret[i] = grub_xasprintf ("%s:gw %s/%d gw %s", dev->card_name, subnet, mask_size, gateway);
      else if (!u32_subnet && u32_gateway)
	ret[i] = grub_xasprintf ("%s:default %s/%d gw %s", dev->card_name, subnet, mask_size, gateway);
      grub_free (subnet);
      grub_free (gateway);
      grub_free (mask);
    }
