...
subprocess.run(["git", "config", "--global", "user.email", "6031763+stsdc@users.noreply.github.com"])
subprocess.run(["git", "config", "--global", "user.name", "stsdc"])
subprocess.run(["git", "config", "--global", "--add", "safe.directory", "/__w/os-patches/os-patches"])
...
for pocket in pockets:
    found_sources = ubuntu_archive.getPublishedSources(
        exact_match=True,
        source_name=component_name,
        status="Published",
        pocket=pocket,
        ...
        if apt_pkg.version_compare(pocket_version, patched_version) > 0:
            ...
            if not github_issue_exists(issue_title):
                ...
                url = "%s/+files/%s_%s.dsc" % (
                        ...
                        found_sources[0].source_package_name,
                        pocket_version,
                    )
